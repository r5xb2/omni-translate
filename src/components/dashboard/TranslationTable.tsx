...
function sanitizeChineseCell(text: string, translationEnabled: boolean): string {
  const compact = text.trim()
  if (!compact) {
    return translationEnabled ? '（待翻譯）' : '（已關閉翻譯）'
  }
  return compact
}

function sanitizeOriginalCell(text: string): string {
  return text.trim() || '（無內容）'
}

export function TranslationTable() {
  const messages = useAppStore((s) => s.messages)
  const sessionStart = useAppStore((s) => s.sessionStartTime) ?? Date.now()
  const { displayMode, enableTranslation, speakerDiarizationEnabled, interactionMode } = useConfigStore((s) => s.config)
  const showSpeakerColumn = speakerDiarizationEnabled && interactionMode === 'conversation'
  const bottomRef = useRef<HTMLDivElement>(null)

  const visibleMessages = useMemo(() => {
    if (messages.length <= MAX_RENDER_ROWS) return messages
    return messages.slice(-MAX_RENDER_ROWS)
  }, [messages])
      ...
      <table className="w-full text-sm border-collapse">
        <thead className="sticky top-0 bg-gray-50 z-10">
          <tr>
            ...
            {showSpeakerColumn && (
              <th className="w-24 px-3 py-2 text-left text-xs font-medium text-gray-500 border-b">講者</th>
            )}
            {displayMode !== 'translated' && (
              <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 border-b">
                {displayMode === 'bilingual' ? '原文' : '內容'}
              </th>
            )}
            {displayMode !== 'original' && (
              <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 border-b">
                {displayMode === 'bilingual' ? '翻譯' : '內容'}
              </th>
            )}
          </tr>
        </thead>
          ...
          {visibleMessages.map((msg, idx) => {
            ...
            return (
              <Fragment key={msg.id}>
                {showSpeakerHint && (
                  <tr>
                    <td colSpan={displayMode === 'bilingual' ? (showSpeakerColumn ? 4 : 3) : (showSpeakerColumn ? 3 : 2)} className="px-3 py-1">
                ...
                <tr key={msg.id} className="border-b hover:bg-gray-50 transition-colors">
                  ...
                  {showSpeakerColumn && (
                    <td className="px-3 py-2 text-xs text-gray-600 whitespace-nowrap align-top">
                      {msg.speakerLabel ?? 'Speaker ?'}
                    </td>
                  )}
                  {displayMode !== 'translated' && (
                    <td className="px-3 py-2 align-top text-gray-800">
                      {msg.status === 'transcribing' ? <Skeleton /> : sanitizeOriginalCell(msg.originalText)}
                    </td>
                  )}
                  {displayMode !== 'original' && (
                    <td className="px-3 py-2 align-top">
                      {msg.status === 'transcribing' || msg.status === 'translating' ? (
                        ...
                        <span className="text-gray-900">{sanitizeChineseCell(msg.translatedText, enableTranslation)}</span>
                      )}
                    </td>
                  )}
                </tr>
